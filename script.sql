...
numSerieMonitor VARCHAR(20) PRIMARY KEY NOT NULL UNIQUE,
...
numSerieTeclado VARCHAR(29) PRIMARY KEY NOT NULL UNIQUE,
idPerifericos INT UNSIGNED,
FOREIGN KEY(idPerifericos) REFERENCES perifericos(idPerifericos)
ON DELETE CASCADE
ON UPDATE CASCADE
...
numSerieMouse VARCHAR(20) PRIMARY KEY NOT NULL UNIQUE,
idPerifericos INT UNSIGNED,
FOREIGN KEY(idPerifericos) REFERENCES perifericos(idPerifericos)
ON DELETE CASCADE
ON UPDATE CASCADE
...
numSerieMonitor VARCHAR(20),
numSerieTeclado VARCHAR(20),
numSerieMouse VARCHAR(20),
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `proAddMonitor` (IN `numInvEscolar` INT,  IN `numSerieMonitor` VARCHAR (20), IN `marca` VARCHAR (20), IN `modelo` VARCHAR(20), IN `estado` BOOLEAN, IN `idTipoPerifericos` INT)   
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `proAddTeclado` (IN `numInvEscolar` INT,  IN `numSerieTeclado` VARCHAR (20), IN `marca` VARCHAR (20), IN `modelo` VARCHAR(20), IN `estado` BOOLEAN, IN `idTipoPerifericos` INT)   
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `proAddMouse` (IN `numInvEscolar` INT,  IN `numSerieMouse` VARCHAR (20), IN `marca` VARCHAR (20), IN `modelo` VARCHAR(20), IN `estado` BOOLEAN, IN `idTipoPerifericos` INT)   
