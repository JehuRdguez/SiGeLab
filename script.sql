...
SELECT `solicitudcambioe`.`idsolicitudCambioE`, `solicitudcambioe`.`alumno`, `laboratorio`.`nombreLaboratorio`, `solicitudcambioe`.`idGrupo`, `solicitudcambioe`.`razon`, `solicitudcambioe`.`fecha`,`solicitudcambioe`.`estado`
...
INSERT INTO `tipoPerifericos` (`idTipoPerifericos`, `tipoPeriferico`) VALUES (NULL, 'Monitor'), (NULL, 'Teclado'), (NULL, 'Mouse');
 
CREATE TABLE  perifericos (
idPerifericos INT UNSIGNED AUTO_INCREMENT PRIMARY KEY NOT NULL,
numInvEscolar VARCHAR(20) UNIQUE,
...
INSERT INTO `perifericos` (`idPerifericos`, `numInvEscolar`, `numSerie`, `marca`, `modelo`, `estado`, `idTipoPerifericos`) VALUES (NULL, 'Pendiente', NULL, NULL, NULL, '0', NULL);
...
numMonitor INT UNSIGNED,
numTeclado INT UNSIGNED,
numMouse INT UNSIGNED,
...
ON DELETE CASCADE
ON UPDATE CASCADE,
FOREIGN KEY(numMonitor) REFERENCES perifericos(idPerifericos)
ON DELETE CASCADE
ON UPDATE CASCADE,
FOREIGN KEY(numTeclado) REFERENCES perifericos(idPerifericos)
ON DELETE CASCADE
ON UPDATE CASCADE,
FOREIGN KEY(numMouse) REFERENCES perifericos (idPerifericos)
ON DELETE CASCADE
ON UPDATE CASCADE
);
 
INSERT INTO `equipo` (`idEquipo`, `idLaboratorio`, `numInvEscolar`, `numSerieEquipo`, `numMonitor`, `numTeclado`, `numMouse`, `ubicacionEnMesa`, `procesador`, `discoDuro`, `ram`, `estado`) VALUES ('1', NULL, '0', '0', NULL, NULL, NULL, NULL, NULL, NULL, NULL, '0');
...
SELECT DISTINCT `perifericos`.`idPerifericos`,`perifericos`.`numInvEscolar`, `perifericos`.`numSerie`, `perifericos`.`marca`, `perifericos`.`modelo`, `perifericos`.`estado`, `perifericos`.`idTipoPerifericos`
FROM `perifericos`
        	WHERE idTipoPerifericos=1;
...
SELECT `equipo`.`idEquipo`,`laboratorio`.`nombreLaboratorio`, `equipo`.`numInvEscolar`, `equipo`.`numSerieEquipo`, `perifericosMonitor`.`numInvEscolar` AS `numInvEscMon`, `perifericosTeclado`.`numInvEscolar` AS `numInvEscTec`, `perifericosMouse`.`numInvEscolar` AS `numInvEscMou`, `equipo`.`ubicacionEnMesa`, `equipo`.`procesador`, `equipo`.`discoDuro`, `equipo`.`ram`, `equipo`.`estado`
FROM `laboratorio`
        	INNER JOIN `equipo` ON `equipo`.`idLaboratorio` = `laboratorio`.`idLaboratorio`
        	INNER JOIN `perifericos` AS `perifericosMonitor` ON `equipo`.`numMonitor` = `perifericosMonitor`.`idPerifericos`
	INNER JOIN `perifericos` AS `perifericosTeclado` ON `equipo`.`numTeclado` = `perifericosTeclado`.`idPerifericos`
        	INNER JOIN `perifericos` AS `perifericosMouse` ON `equipo`.`numMouse` = `perifericosMouse`.`idPerifericos`;
